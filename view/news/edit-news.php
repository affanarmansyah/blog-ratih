...
$pageUpdate = $news->pageUpdateNews($_POST, $_FILES);
...
<body>
    ...
    <form action="" method="POST" enctype="multipart/form-data">
        <input type="hidden" name="id" value="<?php echo $pageUpdate['result']['id']; ?>">
        <label for="title">Title:</label>
        <input type="text" id="title" name="title" value="<?php echo $pageUpdate['result']['title']; ?>" required>

        <label for="image">Image URL:</label>
        <img src="<?= $baseUrl ?>/assets/img/<?php echo $pageUpdate['result']['image'] ? $pageUpdate['result']['image'] : 'default-news.png'; ?>" style="width: 50px; height: 50px;">
        <br>
        <input type="file" name="image" style="margin-top: 10px;">

        <label for="description">Description:</label>
        <input type="text" name="description" value="<?php echo $pageUpdate['result']['description']; ?>" required></input>

        <label for="status">Status:</label>
        <select id="status" name="status" required>
            <?php
            $active = $pageUpdate['result']['status'] == 'Active' ? 'selected' : "";
            $inactive = $pageUpdate['result']['status'] == 'Non Active' ? 'selected' : "";
            ?>
        ...
        <select id="category" name="category_id" required>
            ...
            foreach ($pageUpdate['categories'] as $category) {
                $selected = $category['id'] == $pageUpdate['result']['category_id'] ? 'selected' : "";
